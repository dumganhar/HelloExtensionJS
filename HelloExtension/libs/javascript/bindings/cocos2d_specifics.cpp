...
JSBool js_callFunc(JSContext *cx, uint32_t argc, jsval *vp)
{
    ...
    if (argc >= 1) {        
        ...
        ScriptingCore::getInstance()->setReservedSpot(0, proxy->obj, argv[0]);
        if (argc >= 2) {
            ScriptingCore::getInstance()->setReservedSpot(1, proxy->obj, argv[1]);
            if (argc == 3) {
                ScriptingCore::getInstance()->setReservedSpot(2, proxy->obj, argv[2]);
            }
        }
